...
- [ ] Go — omni repro from `reports/eval-2026-07-02-v1.1.2/corpus-experiments.md`: run the exact lowercase `--root` invocation; score must equal the correct-case run (40/poor), `owner_source: codeowners`
- [ ] Go — a throwaway `config init` on a fresh clone (e.g. `~/workspace/spotinfo` copy in scratch): generated rules parse, layer rule fires on an injected back-edge
- [ ] TypeScript — `config init` smoke on `~/workspace/storybook` subdir copy: parses strict, analyze exit 0
- [ ] Python — `~/workspace/ccgram`: no same-module false positives from public_api_only; analyze exit unchanged vs pre-wave
- [ ] Rust — `~/workspace/herdr`: analyze exit unchanged; no new findings from this wave's rules
- [ ] leave every corpus repo clean (`git status --porcelain` unchanged); run `make all`; open PR
