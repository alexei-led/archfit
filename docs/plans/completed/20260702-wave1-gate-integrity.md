...
- [x] update `docs/design/` decision note: why direction-aware deltas, why BandNA never gates — new `docs/archived/design/20260702-gate-integrity.md` recording all four wave-1 decisions: polarity lives in the metric result not config; knobs wired + `max_new_high` deleted (validated-but-inert is the disease); coupling.gate evaluated inside runPipeline with the projection in cmd (layer ring); abstain ≠ fail rationale
