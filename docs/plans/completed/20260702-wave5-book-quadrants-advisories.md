...
- Review finding (ceiling, deliberate for now): a clone pair whose modules DO share an import edge is skipped by `CloneOnlyPairs`, but the symmetric upgrade only raises `functional`/`unknown` edges — so clone evidence on a pair connected solely by contract/model/intrusive-strength or pinned edges surfaces nowhere. Revisit if real duplicated-knowledge cases hide behind contract edges (would need edge-level upgrade-eligibility in `connectedModulePairs` or a second advisory form).
- Review finding (pre-existing, needs a design decision): `RuleDef.id` is optional at runtime and the JSON schema now mirrors that; two id-less rules of the same type matching the same edge produce colliding finding fingerprints (`sha256(ruleID+from+to+kind)`), and a waiver naming a rule id never matches an id-less rule's findings. Candidate fix: reject empty `id` at config load (breaking) or default it deterministically (`<type>#<index>` — shifts fingerprints for existing id-less baselines).
