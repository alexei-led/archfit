...
- [x] record results into `docs/plans/completed/gap-closure-phase1.md` (no checkbox files outside plan)
...
- [x] add `internal/score/` that synthesizes metrics + gates into the architect's 7 dimensions (`boundary_integrity`, `coupling_balance`, `dependency_graph_health`, `cohesion_modularity`, `change_locality`, `architecture_fitness`, `analysis_confidence`) with value/band/confidence/evidence-refs aligned to `scorecard.yaml` — `score.Synthesize(diagnostic.Diagnostic) Scorecard` is a **pure** decision over an already-computed Diagnostic (no tools, no I/O, no LLM); added to the **core ring** (`internal/arch_test.go`) so the compiler forbids it importing os/exec/yaml/adapters. 0-100 bands match `scorecard.yaml` rubric_version 1 (critical 0-20 → strong 81-100). Each non-meta dimension carries ≥1 evidence ref; `analysis_confidence` is the meta dimension (scores the review). Overall = mean of the six non-meta values
- [x] derive `coupling_balance` strictly from Vlad's balance rule over BC edges (strength×distance×volatility maintenance-effort distribution + worst-case high/high/high count), not a generic metric average; `cohesion_modularity` must treat high-strength+low-distance as healthy cohesion — coupling_balance reads only the `bc/imbalanced_coupling` advisory rollups (parsing `strength`/`distance`/`volatility`/`score_value`/`score_band`/`group_count` from `MatchedBy`): value = `(1 − weighted-mean-maintenance-effort/10)×100`, weighted by each rollup's edge count; a worst-case high/high/high edge (critical band — distributed-monolith risk) caps the dimension at mixed (≤60), and pervasive worst-case (≥5% of edges) caps it at poor (≤40). Cohesion never reaches the advisory pass (classifier scores it `SeverityNone`), so cohesion_modularity penalises only god modules / hidden coupling / cross-module duplication and explicitly never penalises high-strength+low-distance — stated in the dimension summary
- [x] add the `score` command and `--format scorecard`; deterministic output — `cmd/archfit/score.go` (`score` ≡ check --advisory --report --format scorecard; `--base` enables the change_locality delta); `--format scorecard` wired into `check` (enum + render case in `check.go`), which force-enables advisory mode when scorecard is requested so the BC edges reach the synthesis. New renderer `internal/output/scorecard/` delegates synthesis to `internal/score` and formats deterministic Markdown. Smoke-tested on the archfit repo: overall 58/100 (mixed), JSON-free text double-run byte-identical
- [x] write tests enforcing band-matches-value, evidence-per-score, low-confidence caps; add a golden for the scorecard format — `score_test.go`: `TestSynthesize_RuleInvariants` (7 dims, band-matches-value, evidence-per-non-meta, high_quality_requires_confidence, overall mean), `TestSynthesize_Deterministic`, `TestLowConfidenceCap` (low confidence → capped to mixed), `TestCouplingBalance` (no-edges / balanced / single worst-case → ≤mixed / pervasive worst-case → ≤poor), `TestBoundaryIntegrity_GateViolations`, `TestChangeLocality_Unmeasured`, `TestAnalysisConfidence_SemanticToolsAbsent`. `scorecard_test.go`: byte-exact inline **golden** (`TestRenderer_Golden`) + determinism double-run + Format + empty-diagnostic header presence. Coverage: score 89.8%, scorecard renderer 100%
- [x] run `make test` + `make lint` — passes (race, full suite); `make lint` 0 issues; core-ring import (`TestArchImports`, now covering `internal/score`) + golden (`TestGolden`) gates green
...
- [x] add `internal/metrics/modularity/cohesion.go`: edge-density LCOM proxy from the SCIP symbol graph (document the Python `enclosing_range` caveat) — new `CohesionMetric` (`cohesion_lcom`) builds, per measurable module, the undirected graph of its definition symbols connected by **same-module** references and counts connected components (LCOM4): 1 component = cohesive, >1 = fragmented. Distinct from the removed `cohesion_spread` (outbound dispersion) — this reasons over **intra**-module connectivity. Required a new `symbol.Graph.IntraRefs` adjacency emitted by `scip_reader.py` (`_compute_symbols` now returns `intra_refs` alongside `symbol_refs`; same document-scoped attribution). **enclosing_range caveat** documented in `symbol.go`, `scip_reader.py`, and the metric: SCIP indexers don't populate `enclosing_range` → attribution is document-scoped → within-document edges are over-connected, so the proxy trusts only cross-document structure and measures only modules spanning ≥2 documents (and ≥4 symbols). Single-file Python/TS modules are therefore **unmeasurable → n/a, never a false verdict**
- [x] gate shipping on an eval comparing it to expert cohesion judgments on the three repos; if it fails the eval, keep it report-only/disabled (the prior `cohesion_spread` attempt failed — do not repeat) — **live eval run** (`docs/plans/completed/gap-closure-task20-cohesion-eval.md`): Go (archfit) 1 fragmented of 36 measurable packages (the one flagged was `internal/config_test` — test-package noise); ccgram (Python) **n/a** (0 measurable — single-document modules); codegraph (TS) **n/a** (scip absent, no node_modules). Expert cohesion bands were 70/65/42 serviceable/serviceable/mixed. **FAILS the eval**: blind for Python+TS (exactly where the existing LOC-skew proxy diverged from the expert), emits a fragmentation count not a comparable 0-100 band, and cannot see sub-package cohesion. Kept **report-only** (band `info`, never gates) and **disabled in archfit's own `.archfit.yaml`** (self-scan/golden unchanged) — retained rather than removed because the Go-package fragmentation signal is honest where it applies
- [x] write tests + the eval harness; record the eval result in plan notes — `cohesion_test.go`: n/a-when-no-SCIP, n/a-when-no-intra-edges, **single-doc-modules-unmeasurable** (encodes the Python/TS blindness the eval rests on), cohesive-multi-doc → 0 fragmented, fragmented-multi-doc → flagged with part count, small-module-excluded, determinism double-run, and `lcomComponents` union-find unit. `scip_reader_test.py` + `symbols_test.go` assert `intra_refs`/`IntraRefs` parse. Eval result + reproduce commands recorded in plan notes
- [x] run `make test` — must pass before next task — passes (race, full suite); `make lint` 0 issues; core-ring import (`TestArchImports`) + golden (`TestGolden`) gates green. Self-check exit-0, JSON double-run byte-identical, `cohesion_lcom` absent from self-scan (disabled). Metric registry assertions bumped 18→19 (metrics_test/engine_test)

### Task 21: Verify acceptance criteria (final gap-closure gate)

- [x] verify every Phase-1/2 correctness assertion still holds on all three repos — 17/17 PASS via `/tmp/verify_task21.sh`: ccgram delta `change_locality`=**340** (not false-0), codegraph martin lists no node builtins, ccgram **58** BC rollups each carrying `group_count`+`score_value`+`score_band`. Recorded in `docs/plans/completed/gap-closure-task21-acceptance.md`
