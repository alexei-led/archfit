...
- `cmd/archfit/integration_reachability_test.go` — add or extend with testability
  ...
  that testability becomes `measured` with full module coverage and valid sidecar.
  Verify the named subtest(s) for full and partial coverage exist and pass.

Preconditions: Tasks 8–9 merged.

Postconditions: with a fresh, sidecar-validated, fully-mapped source, `testability`
...
# materializeFixture(t, withCoverage=true), exercising collection, sidecar validation
...
| Reachability fixture had no fresh, sidecar-validated coverage, so Task 10 could never be tested | fixture now ships `coverage.out` + attestation sidecar, regenerated by the test setup           |
