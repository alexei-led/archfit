...
| H2 (storybook TS18003 crash)                      | 1 + 5            | Fix group 4.3 + Fix group 1                                        | Two independent causes fixed the crash (exit 3 to 0), but extraction still produced 0 edges -- see H2b below for the third, deeper cause                                                                                                                                                                                                                                                        |
| H2b (storybook TS18003 persists post-crash-fix)   | 6                | `resolveTSConfig` absolute path in subtree mode (commit `038366e`) | depcruise resolves `--ts-config`'s include/exclude relative to CWD, not the tsconfig's own dir. Found only after H2's two fixes landed and a live storybook run still returned 0 edges. Reproduced with raw `bunx depcruise` (relative path -> TS18003, absolute -> 3669 modules); verified end-to-end (coupling_balance 48/mixed, was unreachable). No-subtree path untouched (byte-identical) |
| npx/bunx package-name ambiguity                   | 6                | Both launchers pinned to `dependency-cruiser` (commit `80534bd`)   | `npx depcruise` with no local install resolves to an unrelated npm-registry decoy, exits 0, prints garbage instead of erroring. Found while investigating H2b, not itself the cause (bunx, tried first, already resolves correctly) -- latent risk in npx-only environments, fixed as low-risk hardening                                                                                        |
| H3 (tokio owner_source=none)                      | 6                | Fix group 6.1                                                      | Empirically timed at 14.3s/15s margin. tokio has 1087 distinct git authors -- confirmed NOT a single-committer collapse; the resulting same-owner-dominant distance distribution reflects the known C1 dominant-owner-per-module coarseness, not a resolution bug                                                                                                                               |
