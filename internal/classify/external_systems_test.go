...
func TestRun_ExternalSystems(t *testing.T) {
	...
	tests := []struct {
		name        string
		systems     map[string]config.ExternalSystemDef
		edge        graph.Edge
		...
		},
		{
			...
			systems: map[string]config.ExternalSystemDef{extSysAWS: {Targets: []string{"node_modules/@aws-sdk/**"}, Volatility: extVolMedium}},
			edge: graph.Edge{
		...
		},
		{
			...
			systems: map[string]config.ExternalSystemDef{"serde": {Targets: []string{"serde"}, Volatility: extVolHigh}},
			edge: graph.Edge{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			idx := classify.Run(g, config.ClassifyConfig{Modules: modules, ExternalSystems: tt.systems})
	...
	t.Run("nothing declared — behavior unchanged", func(t *testing.T) {
		...
		g := makeGraph([]graph.Edge{edge})
		cl := classify.Run(g, config.ClassifyConfig{Modules: modules})[edgeKey(edge)]
		if cl.Distance != coupling.DistanceUnknown {
