...
func TestNew_ZeroValue(t *testing.T) {
	...
	if d.SchemaVersion != report.SchemaVersion {
		t.Errorf("schema_version = %q; want %q", d.SchemaVersion, report.SchemaVersion)
	}
...
func TestVerdict_Constants(t *testing.T) {
	tests := []struct {
		...
		{report.VerdictPass, "pass"},
		{report.VerdictFail, "fail"},
		{report.VerdictWarn, gateWarn},
	}
...
func TestSemanticStrengthOverlay_JSONFieldNames(t *testing.T) {
	overlay := report.SemanticStrengthOverlay{
...
func TestDiagnostic_CoverageGapsRoundTrip(t *testing.T) {
	d := report.NewDocument()
	d.CoverageGaps = []report.CoverageGap{
...
func TestDiagnostic_CoverageGapsOmitEmpty(t *testing.T) {
	d := report.NewDocument() // no gaps, no warnings
...
func TestCoverageStatusConstants(t *testing.T) {
	cases := []struct {
		...
		{"StatusOK", report.StatusOK, "ok"},
		{"StatusPartial", report.StatusPartial, "partial"},
		{"StatusAbsent", report.StatusAbsent, "absent"},
		{"StatusDisabled", report.StatusDisabled, "disabled"},
	}
...
func TestDiagnostic_SyntaxFactsRoundTrip(t *testing.T) {
	d := report.NewDocument()
	d.SyntaxFacts = []report.SyntaxFact{
...
func TestSortSyntaxFacts_StableOnTies(t *testing.T) {
	...
	f1 := report.SyntaxFact{File: "x.go", StartLine: 1, Kind: kindFunction, Name: "F", Language: "go"}
	f2 := report.SyntaxFact{File: "x.go", StartLine: 1, Kind: kindFunction, Name: "F", Language: "typescript"} // different language, same keys

	facts := []report.SyntaxFact{f1, f2}
	report.SortSyntaxFacts(facts)
