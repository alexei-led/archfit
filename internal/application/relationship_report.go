...
func attachRelationshipEvidence(diag *result.Result, in relationship.AnalysisEvidence) {
	...
	if diag.ClassifiedEdges == nil {
		diag.ClassifiedEdges = projectRelationshipSummary(in.ClassifiedEdges)
	}
	if diag.ClassifiedEdges != nil {
...
func projectRelationshipSummary(in *relationship.ClassifiedEdgeSummary) *result.ClassifiedEdgeSummary {
	...
	if in.VolatilityProvenance != nil {
		out.VolatilityProvenance = &result.VolatilityProvenance{
			Declared: in.VolatilityProvenance.Declared, Inherited: in.VolatilityProvenance.Inherited,
			Cascade: in.VolatilityProvenance.Cascade, Undeclared: in.VolatilityProvenance.Undeclared,
		}
	}
	if in.DistanceCompression != nil {
		d := in.DistanceCompression
		out.DistanceCompression = &result.DistanceCompressionSummary{
			CompressedMiddleRungs: d.CompressedMiddleRungs, ImplementedRungs: d.ImplementedRungs,
			OmittedRungs: d.OmittedRungs, DeterministicSplits: d.DeterministicSplits, Rationale: d.Rationale,
		}
	...
	if in.TailRisk != nil {
		t := in.TailRisk
		out.TailRisk = &result.CouplingTailRiskSummary{
			WorstBalance: t.WorstBalance, LowerDecileBalance: t.LowerDecileBalance,
			HighOrWorseEdges: t.HighOrWorseEdges, HighOrWorseSharePct: t.HighOrWorseSharePct,
			CriticalEdges: t.CriticalEdges, DistributedMonolithEdges: t.DistributedMonolithEdges,
			CloneOnlyScored: t.CloneOnlyScored, CloneOnlyHighOrWorseEdges: t.CloneOnlyHighOrWorseEdges,
