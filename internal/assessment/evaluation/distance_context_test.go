...
const (
	...
	// Owner-model labels BuildDistanceContext reports. Duplicated here on
	// purpose: the strings are part of the report contract, so a test that reads
	// the production constant could not catch a rename.
...
func TestBuildDistanceContext_SingleOwnerDegenerate(t *testing.T) {
	...
	got := evaluation.BuildDistanceContext(d, cfg.PolicySnapshot(), 1)
...
func TestBuildDistanceContext_NoOwnerSignal(t *testing.T) {
	...
	got := evaluation.BuildDistanceContext(result.New(), cfg.PolicySnapshot(), 0)
...
func TestBuildDistanceContext_RuntimeAsyncEvidence(t *testing.T) {
	...
	got := evaluation.BuildDistanceContext(d, cfg.PolicySnapshot(), 0)
