...
func TestEncapsulation_NoCrossBoundaryIsNA(t *testing.T) {
	...
	edge := metricstest.Edge{From: nodeA.ID(), To: nodeB.ID(), Kind: metricstest.EdgeKindImports}
	g := metricstest.NewFixture([]metricstest.Node{nodeA, nodeB}, []metricstest.Edge{edge})
	idx := metricstest.Index{
		metricstest.ImportKey(nodeA.ID(), nodeB.ID()): metricstest.Classification{
			...
			Distance: relationship.DistanceSameModule,
		},
	}
	m := boundary.EncapsulationMetric{}
	result := m.Calculate(signal.CommonInput{Relationships: metricstest.Classify(g, idx)})
	if result.Band != "n/a" {
...
func TestEncapsulation_EmptyGraphIsNA(t *testing.T) {
	...
	g := metricstest.NewFixture(
		[]metricstest.Node{{Kind: metricstest.NodeKindFile, Path: pathA}},
		nil,
	)
	m := boundary.EncapsulationMetric{}
...
func TestEncapsulation_KnownRatio(t *testing.T) {
	...
	edgeAB := metricstest.Edge{From: nodeA.ID(), To: nodeB.ID(), Kind: metricstest.EdgeKindImports}
	edgeAC := metricstest.Edge{From: nodeA.ID(), To: nodeC.ID(), Kind: metricstest.EdgeKindImports}

	g := metricstest.NewFixture([]metricstest.Node{nodeA, nodeB, nodeC}, []metricstest.Edge{edgeAB, edgeAC})
	...
	idx := metricstest.Index{
		...
		metricstest.ImportKey(nodeA.ID(), nodeC.ID()): metricstest.Classification{
...
func TestEncapsulation_ZeroContractIsRealNotNA(t *testing.T) {
	...
	edgeAB := metricstest.Edge{From: nodeA.ID(), To: nodeB.ID(), Kind: metricstest.EdgeKindImports}
	g := metricstest.NewFixture([]metricstest.Node{nodeA, nodeB}, []metricstest.Edge{edgeAB})
...
func TestEncapsulation_DeltaComputed(t *testing.T) {
	...
	nodeB := metricstest.Node{Kind: metricstest.NodeKindFile, Path: pathB}
	nodeC := metricstest.Node{Kind: metricstest.NodeKindFile, Path: pathC}
	nodeD := metricstest.Node{Kind: metricstest.NodeKindFile, Path: pathD}
	nodeE := metricstest.Node{Kind: metricstest.NodeKindFile, Path: pathE}

	// 3 contract + 2 intrusive = value 0.6
	edges := []metricstest.Edge{
		{From: nodeA.ID(), To: nodeB.ID(), Kind: metricstest.EdgeKindImports},
		{From: nodeA.ID(), To: nodeC.ID(), Kind: metricstest.EdgeKindImports},
		{From: nodeA.ID(), To: nodeD.ID(), Kind: metricstest.EdgeKindImports},
		{From: nodeA.ID(), To: nodeE.ID(), Kind: metricstest.EdgeKindImports},
		{From: nodeB.ID(), To: nodeC.ID(), Kind: metricstest.EdgeKindImports},
	}
	g := metricstest.NewFixture([]metricstest.Node{nodeA, nodeB, nodeC, nodeD, nodeE}, edges)
	...
	idx := metricstest.Index{
		...
		metricstest.ImportKey(nodeA.ID(), nodeD.ID()): metricstest.Classification{
			Strength: relationship.StrengthContract,
			Distance: relationship.DistanceCrossModuleDiffOwner,
		},
		metricstest.ImportKey(nodeA.ID(), nodeE.ID()): metricstest.Classification{
			Strength: relationship.StrengthIntrusive,
			Distance: relationship.DistanceCrossModuleDiffOwner,
		},
		metricstest.ImportKey(nodeB.ID(), nodeC.ID()): metricstest.Classification{
			Strength: relationship.StrengthIntrusive,
			Distance: relationship.DistanceCrossModuleDiffOwner,
		},
	}
	...
	m := boundary.EncapsulationMetric{}
	result := m.Calculate(signal.CommonInput{
		Relationships: metricstest.Classify(g, idx),
		Baseline:      baseline,
	})
...
func TestEncapsulation_AllUnknownIsNA(t *testing.T) {
	...
	idx := metricstest.Index{
		metricstest.ImportKey(nodeA.ID(), nodeB.ID()): metricstest.Classification{
			Strength: relationship.StrengthUnknown,
			Distance: relationship.DistanceCrossModuleDiffOwner,
		},
		metricstest.ImportKey(nodeA.ID(), nodeC.ID()): metricstest.Classification{
			Strength: relationship.StrengthUnknown,
...
func TestEncapsulation_UnknownExcludedFromDenominator(t *testing.T) {
	...
	cross := relationship.DistanceCrossModuleDiffOwner
	idx := metricstest.Index{
		metricstest.ImportKey(nodeA.ID(), nodeB.ID()): {Strength: relationship.StrengthContract, Distance: cross},
		metricstest.ImportKey(nodeA.ID(), nodeC.ID()): {Strength: relationship.StrengthIntrusive, Distance: cross},
		metricstest.ImportKey(nodeA.ID(), nodeD.ID()): {Strength: relationship.StrengthUnknown, Distance: cross},
		metricstest.ImportKey(nodeA.ID(), nodeE.ID()): {Strength: relationship.StrengthUnknown, Distance: cross},
	}
...
func TestEncapsulation_DeclaredExternalExcludedFromDenominator(t *testing.T) {
	...
	g := metricstest.NewFixture([]metricstest.Node{nodeA, nodeB, nodeC, nodeD}, edges)

	cross := relationship.DistanceCrossModuleDiffOwner
	idx := metricstest.Index{
		metricstest.ImportKey(nodeA.ID(), nodeB.ID()): {Strength: relationship.StrengthContract, Distance: cross},
		metricstest.ImportKey(nodeA.ID(), nodeC.ID()): {Strength: relationship.StrengthIntrusive, Distance: cross},
		metricstest.ImportKey(nodeA.ID(), nodeD.ID()): {Strength: relationship.StrengthContract, Distance: relationship.DistanceExternal},
	}
...
func TestEncapsulation_FunctionalAndModelExcluded(t *testing.T) {
	...
	nodes := []metricstest.Node{
		{Kind: metricstest.NodeKindFile, Path: pathA},
		{Kind: metricstest.NodeKindFile, Path: pathB},
		{Kind: metricstest.NodeKindFile, Path: pathC},
		{Kind: metricstest.NodeKindFile, Path: pathD},
		{Kind: metricstest.NodeKindFile, Path: pathE},
	}
	from := nodes[0].ID()
	var edges []metricstest.Edge
	for i := 1; i < 5; i++ {
		edges = append(edges, metricstest.Edge{From: from, To: nodes[i].ID(), Kind: metricstest.EdgeKindImports})
	}
	// pathE reused as a 5th target via a second source to get a 5th edge.
	edges = append(edges, metricstest.Edge{From: nodes[1].ID(), To: nodes[4].ID(), Kind: metricstest.EdgeKindImports})
	g := metricstest.NewFixture(nodes, edges)

	cross := relationship.DistanceCrossModuleDiffOwner
	idx := metricstest.Index{
		metricstest.ImportKey(from, nodes[1].ID()):          {Strength: relationship.StrengthContract, Distance: cross},
		metricstest.ImportKey(from, nodes[2].ID()):          {Strength: relationship.StrengthIntrusive, Distance: cross},
		metricstest.ImportKey(from, nodes[3].ID()):          {Strength: relationship.StrengthFunctional, Distance: cross},
		metricstest.ImportKey(from, nodes[4].ID()):          {Strength: relationship.StrengthFunctional, Distance: cross},
		metricstest.ImportKey(nodes[1].ID(), nodes[4].ID()): {Strength: relationship.StrengthModel, Distance: cross},
	}
...
func TestEncapsulation_LowCoverageCapsGoodBand(t *testing.T) {
	...
	const ncross = 10
	nodes := make([]metricstest.Node, ncross+1)
	for i := range nodes {
		nodes[i] = metricstest.Node{Kind: metricstest.NodeKindFile, Path: "pkg/m" + strconv.Itoa(i) + "/f.go"}
	}
	from := nodes[0].ID()
	cross := relationship.DistanceCrossModuleDiffOwner
	var edges []metricstest.Edge
	...
	for i := 1; i <= ncross; i++ {
		edges = append(edges, metricstest.Edge{From: from, To: nodes[i].ID(), Kind: metricstest.EdgeKindImports})
		var st relationship.Strength
		switch {
		case i <= 3:
			st = relationship.StrengthContract
		case i == 4:
			st = relationship.StrengthIntrusive
		default:
			st = relationship.StrengthUnknown
		}
		idx[metricstest.ImportKey(from, nodes[i].ID())] = metricstest.Classification{Strength: st, Distance: cross}
	}

	g := metricstest.NewFixture(nodes, edges)
	result := boundary.EncapsulationMetric{}.Calculate(signal.CommonInput{Relationships: metricstest.Classify(g, idx)})
...
func TestEncapsulation_NoIntrusiveIsNA(t *testing.T) {
	...
	idx := metricstest.Index{
		metricstest.ImportKey(nodeA.ID(), nodeB.ID()): {Strength: relationship.StrengthContract, Distance: cross},
		metricstest.ImportKey(nodeA.ID(), nodeC.ID()): {Strength: relationship.StrengthContract, Distance: cross},
	}
