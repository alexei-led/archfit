...
func metric(name string, value float64, band, conf string) result.MetricResult {
...
func TestCouplingBalance_EmptyEdges(t *testing.T) {
	...
	t.Run("degenerate import graph + scored clone-only evidence still measures", func(t *testing.T) {
		degen := metricIndex{metricBlastRadius: metric(metricBlastRadius, 0, "n/a", "low")}
		summary := &result.ClassifiedEdgeSummary{
			Scored:           1,
			MeanBalance:      6,
			CloneOnlyScored:  1,
			ConnectedModules: 2,
	...
	t.Run("blast_radius disabled by config is NOT degeneracy — scored summary still measures", func(t *testing.T) {
		summary := &result.ClassifiedEdgeSummary{Scored: 10, MeanBalance: 8.2}
	...
	t.Run("no edges never strong through Synthesize", func(t *testing.T) {
		d := result.New()
		d.Metrics = []result.MetricResult{metric("coverage", 0.2, "poor", "low")}
...
func TestCouplingBalance_Distribution(t *testing.T) {
	...
	summary := func(scored, abstained int, meanBalance float64, bySev map[string]int) *result.ClassifiedEdgeSummary {
	...
	cases := []struct {
		name       string
		sum        *result.ClassifiedEdgeSummary
		wantMinVal int
		wantMaxVal int
		wantConf   Confidence
		wantBand   Band
		...
		},
		{
			...
			sum: func() *result.ClassifiedEdgeSummary {
...
func TestCouplingBalance_ExplainsCapAndCriticalWording(t *testing.T) {
	nonDegen := nonDegenMetricIndex()
	capped := &result.ClassifiedEdgeSummary{
		Scored:      10,
		MeanBalance: 7,
	...
	uncapped := &result.ClassifiedEdgeSummary{
		Scored:      400,
		MeanBalance: 4.61,
	...
	distributed := &result.ClassifiedEdgeSummary{
		Scored:              10,
		MeanBalance:         7,
		BySeverity:          map[string]int{sevCritical: 1},
...
func TestCouplingBalance_TinyFullyScoredGraphCapsConfidence(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			d := result.New()
			d.Metrics = []result.MetricResult{metric(metricBlastRadius, 1, "info", "high")}
...
func TestCouplingBalance_TailRiskEvidenceString(t *testing.T) {
	...
	sum := &result.ClassifiedEdgeSummary{
		...
		CloneOnlyScored:   4,
		CloneOnlyAdvisory: 1,
		TailRisk: &result.CouplingTailRiskSummary{
			WorstBalance:              3,
...
func TestCouplingBalance_VolatilityProvenanceEvidence(t *testing.T) {
	nonDegen := nonDegenMetricIndex()
	base := func() *result.ClassifiedEdgeSummary {
		return &result.ClassifiedEdgeSummary{
			Total: 10, Scored: 10, MeanBalance: 9.0,
			BySeverity: map[string]int{sevLow: 10},
		}
	}
	...
	t.Run("provenance counts disclosed in evidence", func(t *testing.T) {
		sum := base()
		sum.VolatilityProvenance = &result.VolatilityProvenance{Declared: 2, Inherited: 179}
		got := couplingBalance(nil, nonDegen, sum)
		found := false
		for _, ev := range got.Evidence {
	...
	t.Run("undeclared remainder disclosed when nonzero", func(t *testing.T) {
		sum := base()
		sum.VolatilityProvenance = &result.VolatilityProvenance{Declared: 1, Cascade: 3, Undeclared: 4}
		got := couplingBalance(nil, nonDegen, sum)
		found := false
		for _, ev := range got.Evidence {
