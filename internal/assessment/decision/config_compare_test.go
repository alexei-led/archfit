...
// Coverage names crossing this boundary. They are restated here rather than
// read from an extractor: assessment must not import an adapter, even in a
// test. internal/coverage_names_test.go pins these spellings against the real
// adapters, so a rename on either side fails there.
//
...
const (
	...
	toolGoPackagesName = primaryTool
	toolDepCruiserName = "dependency-cruiser"
	toolGrimpName      = "grimp"
)
