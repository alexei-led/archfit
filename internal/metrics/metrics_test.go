...
func TestNew_ReturnsAllMetrics(t *testing.T) {
	...
	if len(ms) != 22 {
		t.Errorf("expected 22 metrics got %d", len(ms))
	}
	names := make(map[string]bool)
	for _, m := range ms {
		names[m.Name()] = true
	}
	for _, want := range []string{"encapsulation", "unbalanced_edge", "cycle", "coverage", "blast_radius", "cohesion_lcom", "change_amplification", "hidden_coupling", "structural_weight", "file_structural_weight", "unsafe_density", "struct_field_density", "complexity", metricRiskHub, "architecture_fitness", "functional_candidates", metricChangeLocality, "instability", "abstractness", "martin_distance", "propagation_cost", "change_coupling"} {
		if !names[want] {
...
func TestNew_ExplicitDisableHonored(t *testing.T) {
	...
	ms := metrics.New(cfg)
	if len(ms) != 20 {
		t.Fatalf("expected 20 metrics (22 - 2 disabled), got %d", len(ms))
	}
	for _, m := range ms {
