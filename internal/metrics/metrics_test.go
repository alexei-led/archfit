...
func TestNew_ReturnsAllMetrics(t *testing.T) {
	...
	if len(ms) != 18 {
		t.Errorf("expected 18 metrics got %d", len(ms))
	}
	names := make(map[string]bool)
	for _, m := range ms {
		names[m.Name()] = true
	}
	for _, want := range []string{"encapsulation", "unbalanced_edge", "cycle", "coverage", "blast_radius", "change_amplification", "hidden_coupling", "structural_weight", "complexity", metricRiskHub, "architecture_fitness", "functional_candidates", metricChangeLocality, "instability", "abstractness", "martin_distance", "propagation_cost", "change_coupling"} {
		if !names[want] {
...
func TestNew_ExplicitDisableHonored(t *testing.T) {
	...
	ms := metrics.New(cfg)
	if len(ms) != 16 {
		t.Fatalf("expected 16 metrics (18 - 2 disabled), got %d", len(ms))
	}
	for _, m := range ms {
