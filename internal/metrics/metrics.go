...
// ChangeHistory is an alias for signal.ChangeHistory. The type lives in
// internal/model/signal; this alias keeps the metrics API stable so the 13
// metric implementation files need no import changes.
type ChangeHistory = signal.ChangeHistory

// MetricInput is an alias for signal.MetricInput. The type lives in
// internal/model/signal; this alias keeps the metrics API stable so the 13
// metric implementation files need no import changes.
type MetricInput = signal.MetricInput
