...
func TestBuildCoverageGaps(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			gaps := BuildCoverageGaps(tc.cov, Coverage(tc.cfg), "")
...
func TestBuildCoverageGaps_ProjectMarkerSuppression(t *testing.T) {
	...
	t.Run("pure-Go repo: no cargo or cargo-modules gap", func(t *testing.T) {
		t.Parallel()
		gaps := BuildCoverageGaps(allRustAbsent, Coverage(cfgDefault), goOnlyDir)
		for _, g := range gaps {
	...
	t.Run("mixed Go+Rust repo: cargo gap present", func(t *testing.T) {
		t.Parallel()
		gaps := BuildCoverageGaps(allRustAbsent, Coverage(cfgDefault), mixedDir)
		found := false
		for _, g := range gaps {
			if g.Tool == toolCargo {
				found = true
	...
	t.Run("explicit gate on rust overrides marker suppression", func(t *testing.T) {
		t.Parallel()
		gaps := BuildCoverageGaps([]result.Coverage{
			{Tool: toolCargo, Status: result.StatusAbsent},
		}, Coverage(cfgRustGate), goOnlyDir)
		found := false
		for _, g := range gaps {
			if g.Tool == toolCargo {
				found = true
	...
	t.Run("explicit gate on cargo-modules overrides marker suppression", func(t *testing.T) {
		...
		gaps := BuildCoverageGaps([]result.Coverage{
			{Tool: toolCargoModules, Status: result.StatusAbsent},
		}, Coverage(cfgCargoModulesGate), goOnlyDir)
		found := false
		for _, g := range gaps {
			if g.Tool == toolCargoModules {
				found = true
	...
	t.Run("empty root disables suppression (backward compat)", func(t *testing.T) {
		t.Parallel()
		gaps := BuildCoverageGaps(allRustAbsent, Coverage(cfgDefault), "")
		found := false
		for _, g := range gaps {
			if g.Tool == toolCargo {
				found = true
...
func testGateOffGapDisclosure(t *testing.T) {
	...
	t.Run("disclosed gap never gates, even under --require-tools", func(t *testing.T) {
		t.Parallel()
		diag := result.Diagnostic{CoverageGaps: BuildCoverageGaps(goAbsent, Coverage(cfgGoOff), goOnlyDir)}
...
func testMarkDisabledPrimaries(t *testing.T) {
	...
	t.Run("disabled language primary becomes a disabled row", func(t *testing.T) {
		t.Parallel()
		cov := MarkDisabledPrimaries(absent(), Coverage(off), tsDir)
	...
	t.Run("a language that is not in the tree stays absent", func(t *testing.T) {
		t.Parallel()
		cov := MarkDisabledPrimaries(absent(), Coverage(off), goOnly)
		if cov[0].Status != result.StatusAbsent {
	...
	t.Run("explicit gate keeps the row absent so the gap survives", func(t *testing.T) {
		t.Parallel()
		cov := MarkDisabledPrimaries(absent(), Coverage(offGated), tsDir)
		if cov[0].Status != result.StatusAbsent {
	...
	t.Run("non-primary and non-absent rows are untouched", func(t *testing.T) {
		...
		cov := MarkDisabledPrimaries(in, Coverage(off), tsDir)
	...
	t.Run("a disabled row raises no install gap", func(t *testing.T) {
		t.Parallel()
		cov := MarkDisabledPrimaries(absent(), Coverage(off), tsDir)
		if gaps := BuildCoverageGaps(cov, Coverage(off), ""); len(gaps) != 0 {
	...
	t.Run("an empty root discloses the opt-out", func(t *testing.T) {
		t.Parallel()
		cov := MarkDisabledPrimaries(absent(), Coverage(off), "")
		if cov[0].Status != result.StatusDisabled {
...
func TestBuildConfigWarnings(t *testing.T) {
	...
	t.Run("empty config and no tool errors returns nil", func(t *testing.T) {
		...
		if got := BuildConfigWarnings(Options(cfg).LintWarnings, nil); got != nil {
	...
	t.Run("tool errors appear after lint warnings", func(t *testing.T) {
		...
		got := BuildConfigWarnings(Options(cfg).LintWarnings, toolErrs)
		if len(got) == 0 {
...
func TestConfigToolGate(t *testing.T) {
	...
	for _, tc := range cases {
		if got := ConfigToolGate(Coverage(cfg), tc.tool); got != tc.want {
...
func TestBuildJudgmentDecisionTasks(t *testing.T) {
	...
	t.Run("module with neither subdomain nor volatility emits decision task", func(t *testing.T) {
		...
		tasks := BuildJudgmentDecisionTasks(PolicySnapshot(cfg).Topology.Modules, nil, configPath)
		found := false
	...
	t.Run("approved llm label emits decision task pointing at labels file", func(t *testing.T) {
		...
		tasks := BuildJudgmentDecisionTasks(PolicySnapshot(cfg).Topology.Modules, lbls, configPath)
		found := false
		for _, t2 := range tasks {
...
func TestSkippedPassCoverageRows_ScipDisabled(t *testing.T) {
	...
	gaps := BuildCoverageGaps([]result.Coverage{cov}, Coverage(config.Config{}), "")
	if len(gaps) != 0 {
...
func TestSkippedPassCoverageRows_SyntaxDisabled(t *testing.T) {
	...
	gaps := BuildCoverageGaps(cov, Coverage(config.Config{}), "")
	if len(gaps) != 0 {
